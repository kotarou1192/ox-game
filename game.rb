...
@bot.command :play do |event, game_mode, difficulty, order_to_attack|
  ...
  if game_mode == '1'
    ...
    event.send_message('入力されたゲームモードが不正です。gamemode 1: 2players, 2:single, 3:cpu_vs_cpu')
    break
  end
  ...
  event.send_message("ゲームモード：#{game_mode_text}、難易度は#{difficulty_text}、CPUとの戦闘ではプレイヤーは#{order_to_attack_text}")
  define(game_mode: @game_mode, difficulty: @difficulty, order_to_attack: @order_to_attack)
